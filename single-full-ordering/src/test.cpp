...
int main(int argc, char **argv) {
  ...
  if ((num_data < 100) || (num_data > 1000000)) {
    std::cerr << "Error: number of keys must be >= 100 and <= 1000000" << std::endl;
    return 1;
  }
  ...
  std::thread mb(thread_func, bt, ops, keys, num_data);
  ...
  mb.join();  // Remember to join the measurement thread.
